...
def test_train_predict_model(sample_train_data, schema_provider):
    """
    ...
    predictions = classifier.predict(sample_train_data)
    assert predictions.shape == (sample_train_data.shape[0],)

    proba_predictions = classifier.predict_proba(sample_train_data)
    assert proba_predictions.shape == (sample_train_data.shape[0], 3)

    classifier.save(PREDICTOR_DIR_PATH)
    assert os.path.exists(os.path.join(PREDICTOR_DIR_PATH, 'predictor.joblib'))


def test_save_load_model(tmpdir, classifier, sample_test_data):
    """
    ...
    # Test predictions
    predictions = loaded_clf.predict(sample_test_data)
    assert np.array_equal(predictions, classifier.predict(sample_test_data))

    proba_predictions = loaded_clf.predict_proba(sample_test_data)
...
def test_predict_with_model(sample_train_data, schema_provider):
    """
    ...
    classifier = Classifier(sample_train_data, schema_provider)
    classifier.train()
    predictions = Classifier.predict_with_model(classifier, sample_train_data, return_proba=True)

    assert isinstance(predictions, pd.DataFrame)
    assert predictions.shape[0] == sample_train_data.shape[0]
